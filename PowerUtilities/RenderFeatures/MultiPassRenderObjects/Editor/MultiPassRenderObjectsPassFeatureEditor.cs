...
{
	[CustomPropertyDrawer(typeof(MultiPassRenderObjects.RenderObjectsSettings), true)]
    public class MultiPassRenderObjectsPassFeatureEditor : PropertyDrawer
    {
		public class Styles
	    {
			...
			public static GUIContent multiPassHeader = new GUIContent("Multipass Options");
		    //Filters
		...
		SerializedProperty m_PassName;
		SerializedProperty m_PassCount;
		HeaderBool m_MultiPassOptions;
        ...
        private void Init(SerializedProperty property)
        {
			...
			m_PassCount = property.FindPropertyRelative("passCount");
			m_PassName = property.FindPropertyRelative("passName");
			m_MultiPassOptions = new HeaderBool("MultiPass Options", false);
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
	    {
			...
			// multi pass options
			m_MultiPassOptions.value = EditorGUI.Foldout(rect, m_MultiPassOptions.value, Styles.multiPassHeader, true);
			SaveHeaderBool(m_MultiPassOptions);
			rect.y += Styles.defaultLineSpace;
			if (m_MultiPassOptions.value)
            {
				EditorGUI.indentLevel++;
				DrawMultiPassOptions(ref rect);
				rect.y += Styles.defaultLineSpace;
				EditorGUI.indentLevel--;
            }
        ...
        private void DrawMultiPassOptions(ref Rect rect)
        {
			//EditorGUI.BeginChangeCheck();

			EditorGUI.PropertyField(rect, m_PassCount);
			rect.y += Styles.defaultLineSpace;
			EditorGUI.PropertyField(rect,m_PassName);
			rect.y += Styles.defaultLineSpace;
